...
for each group, run `npx ncu -u -f ...` with the appropriate filters (that's what the `-f` flag is for) to update the version specs in package.json files then `pnpm install` to install the updated packages
